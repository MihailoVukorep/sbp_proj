...
class DatabaseInitializer:
    ...
    def __init__(self, csv_path, connection_string='mongodb://localhost:27017/', 
                 database_name='SBP_DB', collection_name='movies'):
    ...
    def _process_chunk(self, chunk_data):
        ...
        for row_dict in chunk.to_dict('records'):
            try:
                movie = MovieDocument.transform(row_dict)
                movies.append(movie)
            except Exception:
                errors += 1
                continue
    ...
    def verify(self):
        ...
        try:
            ...
            pipeline = [
                {"$match": {"release_info.release_date.year": {"$exists": True}}},
                {"$group": {
                    "_id": None,
                    "min_year": {"$min": "$release_info.release_date.year"},
                    "max_year": {"$max": "$release_info.release_date.year"}
                }}
            ]
            ...
            if sample:
                ...
                print(f"  TMDB ID: {sample.get('tmdb_id')}")
                print(f"  IMDB ID: {sample.get('media', {}).get('imdb_id')}")
                print(f"  Rating: {sample.get('ratings', {}).get('vote_average')}")
                print(f"  Year: {sample.get('release_info', {}).get('release_date', {}).get('year')}")
            else:
